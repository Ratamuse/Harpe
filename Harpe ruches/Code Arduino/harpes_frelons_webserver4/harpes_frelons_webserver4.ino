...
TaskHandle_t harpeTaskHandle;
TaskHandle_t wifiTaskHandle;

void harpeTask(void* parameter) {
  while (true) {
    if (useHarpe1) {
...
void loop() {
  if (startWebServer) {
    ...
    if (lux >= lux_min && humidity0 <= hum_max) {
      if (xTaskCreatePinnedToCore(harpeTask, "Harpe Task", 10000, NULL, 1, &harpeTaskHandle, 0) != pdPASS) {
        Serial.println("Failed to create Harpe Task");
      }
      if (xTaskCreatePinnedToCore(wifiTask, "WiFi Task", 10000, NULL, 1, &wifiTaskHandle, 1) != pdPASS) {
